...
# 엘라스틱서치 전처리
es = Elasticsearch(["http://127.0.0.1:9200"],timeout=60, max_retries=10, retry_on_timeout=True)
...
ht_info = pd.read_csv('./호텔_리스트_최종.csv')
gu_loglat = pd.read_csv('./seoul_gu_loglat.csv')
ht_map = './hotel.json'
...
def result():
    ...
    body ="""
        {"query": {"bool": {"must": [{"match": {"review": "%s"}}], "should": []}}, "size": 50}
    """ % (query_text)

    if gu:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"gu": {"query": gu,"boost": 10.0}}})
        body = json.dumps(body_json)
    if station:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"station": { "query": station,"boost": 10.0} }})
        body = json.dumps(body_json)
    if station:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"station2": {"query": station,"boost": 10.0}}})
        body = json.dumps(body_json)
    if grade:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"grade": {"query": grade,"boost": 10.0}}})
        body = json.dumps(body_json)
    if word1:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"word1": {"query": word1,"boost": 10.0}}})
        body = json.dumps(body_json)
    if word2:
        body_json = json.loads(body)
        body_json['query']["bool"]["should"].append({"match": {"word2": {"query": word2,"boost": 10.0}}})
        body = json.dumps(body_json)
    if word3:
        body_json = json.loads(body)
        body_json["query"]["bool"]["should"].append({"match": {"word3": {"query": word3,"boost": 10.0}}})
        body = json.dumps(body_json)
